...
bool loadMedia()
{
	...
	else
	{
		...
		if (!gStartPromptTexture.loadFromRenderedText("Press s to start / stop the timer.", textColor))
		{
			printf("Failed to render text from texture!\n");
			success = false;
		}
		// pause / unpause prompt texture
		if (!gPausePromptTexture.loadFromRenderedText("Press p to pause / unpause timer.", textColor))
		{
			printf("Failed to render text from texture!\n");
			success = false;
		}
...
{ 
	...
	else 
	{
		...
		// the application timer (lives on the stack)
		LTimer timer;
